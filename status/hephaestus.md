...
- **Doing today**: TASK-0017 r4 commit `48b5a414dc6fe7e143a15589c23ae71e7a0260b8` explicitly confirms existing state-directory durability during open; all local exact-tree gates and independent re-review are green.
- **Blocked**: automatic exact-head CI run `30807587526` is in progress, then merge remains gated on Zeus's r4 `[ack]`; PR #27 stays Draft and unmerged.
- **Next**: monitor automatic CI without triggering it; on success send the r4 exact-head review request with restart fail/stay/recover and absent/empty/populated startup evidence.
- **Recent** (≤5):
