...
worktree or switch a shared clone. Check that current UI/build entries are preserved, the
reviewed ignore block is unchanged, and no legitimate frontend artifact workflow is hidden by
the patterns. Return one explicit r1 verdict: `[ack]` for all six, or `[request-changes]` with
...
your carry verdict rather than silently expanding this task. Once this review returns and the
six independent landings finish, the next honest prerelease train can unblock your remaining
